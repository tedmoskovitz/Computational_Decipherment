...
public class EM 
{
	...
	public EM(In corpus, In rules, In ipa_corp, int its)
	{
		...
		pairs = new Bag<Bigram>(); 
		syms = new char[CHAR_NUM];
		ipa = new char[CHAR_NUM];
		sents = new LinearProbingHashST<Sentence, Sentence>(); 
		iterations = its; 
								...
								if (total > 0) 
									{
										...
										b.Bigram_setTC(tc + (b.Bigram_getProb() / (double)total)); 
									}
							}
						}
			...
			for (int l = 0; l < CHAR_NUM; l++)
			{
				...
				for (int t = 0; t < CHAR_NUM; t++)
				{
					...
					for (Bigram b : pairs)
					{
						...
						if (b.Bigram_equals(sym_s, ipa_s) && (tc != 0.0))
						{
							b.Bigram_setProb(tc / (double)total);
						}
					}
